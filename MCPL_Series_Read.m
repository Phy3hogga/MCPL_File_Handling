...
%% Test Data
%XBD Dataset directory (ensure no other XBD data exists in the same directory)
Directory_Path = 'D:\Data_Capture\xbd_test2';
%MAT filename to export data to
...
Z_Target = 397e-3;
%Find propogation vector for target Z
Prop = (Z_Target - Z)./Dz;
%Calculate propotation points in X,Y,Z from propogation vector
Prop_Z = Prop .* Dz + Z;
Prop_X = Prop .* Dx + X;
Prop_Y = Prop .* Dy + Y;
...
%% Rebin propogated data by event angle within a given sampling grid.
%Number of bins to sample in X and Y
Bins_Num = 250;
%Tolerance for plot limits, ensure all data is within the limits set
Bin_Tol = 0.1e-3;
