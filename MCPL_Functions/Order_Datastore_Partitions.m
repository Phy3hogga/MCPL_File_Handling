...
function File_Chunks = Order_Datastore_Partitions(Datastore_Directory_Path)
    ...
    for Current_Reference = 1:length(Matfile_Partition_References)
        ...
        %Compare sequential files to check they all contain identical variables
        if(Current_Reference == 1)
            First_Matfile_Variable = {Mat_File_Variables.name};
        else
            %First file extra variables
            Variable_Comparison = setdiff(First_Matfile_Variable, {Mat_File_Variables.name});
            if(~isempty(Variable_Comparison))
                error("Order_Datastore_Partitions : Unexpected extra variables found in first partition file.");
            end
            %Comparison file extra variables
            Variable_Comparison = setdiff({Mat_File_Variables.name}, First_Matfile_Variable);
            if(~isempty(Variable_Comparison))
                error("Order_Datastore_Partitions : Unexpected extra variables found in current parition file.");
            end
        end
        if(~contains('Datastore_Partition_Information', {Mat_File_Variables.name}))
            error("Order_Datastore_Partitions : Datastore_Partition_Information missing from Datastore Partition");
        end
        ...
        if(~isfield(Datastore_Partition_Information, 'NumberOfPartitions'))
            error("Order_Datastore_Partitions : Datastore_Partition_Information missing NumberOfPartitions from Datastore Partition Information");
        end
        ...
        if(~isfield(Datastore_Partition_Information, 'PartitionIndex'))
            error("Order_Datastore_Partitions : Datastore_Partition_Information missing PartitionIndex from Datastore Partition Information");
        end
        ...
        if(~isfield(Datastore_Partition_Information, 'BlockIndexInPartition'))
            error("Order_Datastore_Partitions : Datastore_Partition_Information missing BlockIndexInPartition from Datastore Partition Information");
        end
        ...
        if (~(range(Size_1(:)) == 0 && range(Size_2(:)) == 0))
            error("Order_Datastore_Partitions : Mismatch in variable sizes.");
        end
        if(mean(Size_1(:)) == 1)
            if(mean(Size_2(:)) == 1)
            ...
            if(mean(Size_2(:)) == 1)
                ...
                error("Order_Datastore_Partitions : 2D variables found within partition file, expected one dimensional data.");
            end
        end
    ...
    if(range(Number_Of_Partitions) ~= 0)
        error("Order_Datastore_Partitions : Mismatch between Datastore_Partition_Information partitions and the number of partition files.");
    end
    ...
    if(height(Partition_Table) ~= height(Unique_Partition_Table))
        error("Order_Datastore_Partitions : Non-unique parition data between seperate paritions.");
    end
    ...
    if(~Check_Sequential_Numeric_Array(Unique_Partitions))
        error("Order_Datastore_Partitions : Sorted Partitions aren't sequential, missing partition files.");
    end
    ...
    for Current_Partition_Index = 1:length(Unique_Partitions)
        ...
        if(~Check_Sequential_Numeric_Array(Sub_Partitions))
            error(strcat("Order_Datastore_Partitions : Sorted Sub Partitions aren't sequential for partition ", num2str(Unique_Partitions(Current_Partition_Index)), ", missing partition files."));
        end
    end
